<template>
  ...
  <div class="add-content-body">
    ...
    <div class="add-content-body-add" v-if="current===2">
      <n-grid :x-gap="12" :y-gap="8" :cols="2">
        <n-grid-item v-for="item in searchResult" :key="item" @click="select(item)">
          <n-card class="add-content-body-add-card"
                  :class="{selected:(selectWord != null && (selectWord.word+selectWord.katakana) === (item.word+item.katakana))}"
                  :title="item.word">
            {{ item.katakana }}
          </n-card>
        </n-grid-item>
      ...
      <n-select style="margin-top: 20px" v-model:value="selectLetter" :options="letterOptions"
                placeholder="选择单词分类"/>
